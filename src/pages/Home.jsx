  ...
  return (
    <div className='max-w-4xl min-h-svh mx-auto px-4 my-10'>
      ...
      <p className='text-center text-gray-700 mb-8'>
        This app helps you stay organized by tracking your daily tasks. You can add, view, and complete todos easily.
      </p>
      ...
      <div className='flex flex-col sm:flex-row justify-around text-center mb-10'>
        <div className='bg-green-100 p-4 rounded-md shadow-md w-full sm:w-[30%] mb-4 sm:mb-0'>
          ...
          <p className='text-2xl font-bold'>{!userId ? 0 : data?.todos?.length }</p>
        </div>
        <div className='bg-blue-100 p-4 rounded-md shadow-md w-full sm:w-[30%] mb-4 sm:mb-0'>
          ...
          <p className='text-2xl font-bold'>{!userId ? 0 : data?.todos?.filter(todo => todo.completed).length}</p>
        </div>
      ...
      <div className='bg-white shadow-md rounded-md p-6'>
        ...
        {
          !userId ? (
            <p className='text-gray-500 text-center w-full'>Please log in to view your todos summary.</p>
          ) : isLoading ? (
          ...
          <ul className='space-y-2'>
            {data?.todos?.slice(0, 5).map(todo => (
              <li key={todo.id} className='border p-3 rounded hover:bg-amber-50 flex justify-between items-center'>
                <span>{todo.todo}</span>
                <span className={`text-sm px-2 py-1 rounded ${todo.completed ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'}`}>
